...
export default function DualModeChartCard({
  ...
  const referenceMax = referenceAreas ? Math.max(...referenceAreas.map((r) => r.y2)) : -Infinity;

  // standard（時系列）
  const dataValuesStandard = rawData.map((d) => Number(d.value)).filter((v) => !isNaN(v));

  // processed（集計）
  const dataValuesProcessed = processedData.flatMap((d) =>
    processedLines.map((l) => Number(d[l.key])).filter((v) => !isNaN(v)),
  );
  ...
  const yDomainStandardFinal: AxisDomain = enableRef
    ...
    : ([, dataMax]: readonly [number, number]) => [0, dataMax] as readonly [number, number];
